...
const SCRAPING_DIR = isMac
	? `${process.env.HOME}/.pi/sandbox-cache/browser-tools`
	: `${process.env.HOME}/.cache/browser-tools`;

const CHROME_BIN = isMac
	? "/Applications/Google Chrome.app/Contents/MacOS/Google Chrome"
	: (() => {
		for (const bin of ["chromium", "chromium-browser", "google-chrome"]) {
			try { return execSync(`which ${bin}`, { stdio: "pipe" }).toString().trim(); } catch {}
		}
		console.error("✗ No Chromium/Chrome binary found"); process.exit(1);
	})();

const PROFILE_SRC = isMac
