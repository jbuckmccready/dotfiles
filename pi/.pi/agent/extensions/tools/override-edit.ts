...
async function readExistingNormalized(workspace: Workspace, path: string, absolutePath: string): Promise<string> {
	try {
		return normalizeToLF(stripBom(await workspace.readText(absolutePath)).text);
	} catch (err: any) {
...
async function maybeReadNormalized(workspace: Workspace, absolutePath: string): Promise<string | null> {
...
async function buildRowPlan(text: string, cwd: string, workspace: Workspace): Promise<ParsedPlan> {
	const scripts = parseRowScript(text);
	const store = createSnapshotStore(cwd, (path, absolutePath) =>
		readExistingNormalized(workspace, path, absolutePath),
	);
...
async function buildPatchPlan(text: string, cwd: string, workspace: Workspace): Promise<ParsedPlan> {
	const operations = parsePatch(text);
	const store = createSnapshotStore(cwd, (_path, absolutePath) => maybeReadNormalized(workspace, absolutePath));
...
async function buildPlan(text: string, cwd: string, workspace: Workspace): Promise<ParsedPlan> {
	return isPatchLikePayload(text) ? buildPatchPlan(text, cwd, workspace) : buildRowPlan(text, cwd, workspace);
}

async function buildPreviewPlan(text: string, cwd: string, workspace: Workspace, argsComplete: boolean): Promise<ParsedPlan> {
	if (!argsComplete && isPatchLikePayload(text) && !isPatchPayload(text)) {
		return buildPatchPlan(patchTextForPreview(text), cwd, workspace);
	}
	return buildPlan(text, cwd, workspace);
}
...
async function preflightPlan(plan: ParsedPlan, workspace: Workspace, signal?: AbortSignal): Promise<void> {
	for (const change of plan.changes) {
		...
		if (change.kind === "delete") await workspace.checkDeleteAccess(change.absolutePath);
	}
}
...
async function readFileForMutation(workspace: Workspace, absolutePath: string): Promise<{ bom: string; ending: "\r\n" | "\n"; content: string }> {
...
async function applyUpdateChange(change: PlannedFileChange, workspace: Workspace, signal?: AbortSignal): Promise<EditDetailsLike> {
	return withFileMutationQueue(change.absolutePath, async () => {
		throwIfAborted(signal);
		const file = await readFileForMutation(workspace, change.absolutePath);
		throwIfAborted(signal);
		...
		await workspace.writeText(change.absolutePath, file.bom + restoreLineEndings(newContent, file.ending));
		throwIfAborted(signal);
		return detailsForChange(change.path, baseContent, newContent);
	});
}

async function applyWriteChange(change: PlannedFileChange, workspace: Workspace, signal?: AbortSignal): Promise<EditDetailsLike> {
	return withFileMutationQueue(change.absolutePath, async () => {
		throwIfAborted(signal);
		const file = await readFileForMutation(workspace, change.absolutePath);
		...
		await workspace.writeText(change.absolutePath, file.bom + restoreLineEndings(change.newText, file.ending));
		return detailsForChange(change.path, file.content, change.newText);
	});
}

async function applyAddChange(change: PlannedFileChange, workspace: Workspace, signal?: AbortSignal): Promise<EditDetailsLike> {
	return withFileMutationQueue(change.absolutePath, async () => {
		throwIfAborted(signal);
		const existing = await maybeReadNormalized(workspace, change.absolutePath);
...
async function applyDeleteChange(change: PlannedFileChange, workspace: Workspace, signal?: AbortSignal): Promise<EditDetailsLike> {
	return withFileMutationQueue(change.absolutePath, async () => {
		throwIfAborted(signal);
		await workspace.checkWriteAccess(change.absolutePath);
		await workspace.checkDeleteAccess(change.absolutePath);
		const current = await readExistingNormalized(workspace, change.path, change.absolutePath);
...
async function applyPlan(plan: ParsedPlan, workspace: Workspace, signal?: AbortSignal): Promise<UnifiedEditDetails> {
	const appliers = {
	...
	for (const change of plan.changes) {
		throwIfAborted(signal);
		const details = await appliers[change.kind](change, workspace, signal);
...
function requestUnifiedEditPreview(
	...
	void buildPreviewPlan(text, cwd, workspace, argsComplete)
...
export function createEditOverride(sandbox: SandboxAPI) {
	return {
		...
		async execute(_toolCallId: string, params: UnifiedEditParams, signal: AbortSignal | undefined, _onUpdate: unknown, ctx: { cwd: string }) {
			...
			const plan = await buildPlan(text, cwd, workspace);
			try {
				await preflightPlan(plan, workspace, signal);
			} catch (err: any) {
				throw new Error(`Preflight failed before mutating files.\n${err?.message ?? String(err)}`);
			}
			const details = await applyPlan(plan, workspace, signal);
			return { content: [{ type: "text" as const, text: formatSummary(details) }], details };
		},

		renderCall(args: unknown, theme: any, context: RenderContext<UnifiedRenderState>) {
			...
			if (mode !== "minimal" && context.argsComplete) {
